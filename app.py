...
def initiate_data():
    ...
    mat = loadmat('D:/knowledge graph/rescal.py-master/data/dnations.mat')
    entity_datafile = 'D:/knowledge graph/rescal.py-master/data/entities_nation.txt'
    relation_datafile = 'D:/knowledge graph/rescal.py-master/data/relations_nation.txt'
    ...
    np.save("idx_test_rel.npy",idx_test)
    np.save("idx_rel.npy", IDX)
...
def generate_data(cluster):
    ...
    # fsz = int(SZ / FOLDS)
    # offset = 0
    idx_test = np.load("idx_test_rel.npy")
    IDX = np.load("idx_rel.npy")
...
def create_plot( P , entities, relation, distribution_0, entityX, entityY, last, modifyX, modifyY, questionX, questionY):
    ...
    heat_data_test = np.swapaxes(heat_data,0,1)
    for c in last:
        ...
        heat_data[:, c_start: c + 1] = heat_data[:, c_start: c + 1][:, dendro_leaves]

        entities_y = entities_y + [entities[c_start: c + 1][i] for i in dendro_leaves]
    ...
    return fig, entities_x,entities_y
...
def predict():
    ...
    heatmap,entities_x, entities_y = create_plot( matrix, entities, relation, distribution_1[relation], entityX, entityY, last,operation[relation_chosen[-1]]["mX"], operation[relation_chosen[-1]]["mY"], operation[relation_chosen[-1]]["qX"], operation[relation_chosen[-1]]["qY"])
...
def configuration():
    ...
    heatmap, entities_x, entities_y = create_plot( matrix, entities, relation, distribution_1[relation],entityX, entityY, last, modifyX, modifyY, questionX, questionY)
    bar, relation_chosen = create_bar_plot(relations, bar_index, bar_value)
    graphJSON = [heatmap, bar]
    graphJSON = json.dumps(graphJSON, cls=plotly.utils.PlotlyJSONEncoder)
    return render_template('result_2.html', graphJSON=graphJSON, relations_left=relation_left, relations = relations, relation = relation,relation_chosen=relation_chosen)
...
def change_to_true():
    ...
    entities_x = session.get('entities_x', None)
    entities_y = session.get('entities_y', None)
    relations = session.get('relations',None)
    GROUND_TRUTH = np.array(session.get('Ground_truth',None))
    last = session.get('last', None)
    distribution_1 = session.get('distribution_1',None)
    ...
    x = entities_x.index(x)
    y = entities_y.index(y)
    ...
    heatmap, entities_x, entities_y = create_plot( matrix , entities, relation, distribution_1[relation], entityX, entityY,last,operation[relations[relation]]["mX"],operation[relations[relation]]["mY"],operation[relations[relation]]["qX"],operation[relations[relation]]["qY"] )
